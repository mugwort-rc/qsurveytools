...
def main(args):
    ...
    if not os.path.isdir(args.input):
        print("input: {!r} is not directory.".format(args.input))
        return 1
    if not os.path.isdir(args.output):
        print("output: {!r} is not directory.".format(args.output))
        return 1
    ...
    for path, file in locate("*.md", args.input):
        data = open(os.path.join(path, file), encoding="utf-8").read()
        html = tpl.render({
